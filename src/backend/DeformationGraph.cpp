                                              ...
                                              pcl::KdTreeFLANN<pcl::PointXYZRGBNormal> & graphTree)
{
    ...
    for(unsigned int i = 0; i < graphCloud->size(); i++)
    {
        ...
        while(numNeighboursAdded < k)
        {
            ...
            if(abs(static_cast<long>(graphTimeMap->at(pointIdxNKNSearch.at(j)) - graphTimeMap->at(i)) < 60000000))
            {
                graphNodes[i].neighbours.push_back(&graphNodes[pointIdxNKNSearch.at(j)]);
                numNeighboursAdded++;
            }
                                         ...
                                         std::vector<uint64_t> & sampledGraphTimes)
{
    ...
    for(unsigned int i = lastPointCount; i < sourceVertices->size(); i++)
    {
        ...
        if(abs(static_cast<long>(sampledGraphTimes[imin] - vertexTime)) <= abs(static_cast<long>(sampledGraphTimes[imid] - vertexTime)) &&
           abs(static_cast<long>(sampledGraphTimes[imin] - vertexTime)) <= abs(static_cast<long>(sampledGraphTimes[imax] - vertexTime)))
        {
            foundIndex = imin;
        }
        else if(abs(static_cast<long>(sampledGraphTimes[imid] - vertexTime)) <= abs(static_cast<long>(sampledGraphTimes[imin] - vertexTime)) &&
                abs(static_cast<long>(sampledGraphTimes[imid] - vertexTime)) <= abs(static_cast<long>(sampledGraphTimes[imax] - vertexTime)))
        {
            foundIndex = imid;
        }
        else
                                                ...
                                                std::vector<uint64_t> * graphTimeMap)
{
    ...
    for(unsigned int i = 0; i < sourceVertices->size(); i++)
    {
        ...
        {
            if(abs(static_cast<long>(graphTimeMap->at(pointIdxNKNSearch.at(j)) - vertexTimeMap->at(i)) < 60000000))
            {
